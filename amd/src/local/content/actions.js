 ...
 * Actions
 *
 * @module     format_mawang/local/content/actions
...
export default class extends BaseComponent {
    ...
    create() {
        ...
        this.name = 'content_actions_mawang';
        // Default query selectors.
        this.selectors = {
            ACTIONLINK: `[data-action-mawang]`,
    ...
    _dispatchClick(event) {
        ...
        const methodName = this._actionMethodName(target.getAttribute('data-action-mawang'));
    ...
    _requestDelete(target, event) {
        ...
        if (cmList.length || sectionInfo.hassummary || sectionInfo.rawtitle || sectionInfo.children.length) {
            getStrings([
                ...
                {key: 'confirmdelete', component: 'format_mawang'},
    ...
    _requestMergeup(target, event) {
        ...
        if (cmList.length || sectionInfo.hassummary || sectionInfo.rawtitle || sectionInfo.children.length) {
            getStrings([
                {key: 'confirm', component: 'moodle'},
                {key: 'confirmmerge', component: 'format_mawang'},
                {key: 'yes', component: 'moodle'},
                {key: 'no', component: 'moodle'}
            ]).done((strings) => {
                Notification.confirm(
    ...
    _requestMoveSection(target, event) {
        ...
        ModalFactory.create({
            ...
            .then(modal => {
                ...
                Templates.render('format_mawang/local/content/movesection', data).
                then((body) => {
                    modal.setBody(body);
    ...
    _requestMoveCm(target, event) {
        ...
        ModalFactory.create({
            ...
            .then(modal => {
                // eslint-disable-next-line promise/no-nesting
                Templates.render('format_mawang/local/content/movecm', data).
                then((body) => {
                    modal.setBody(body);
