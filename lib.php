...
class format_mawang extends core_courseformat\base {
            ...
            'collapsed' => [
                ...
                'element_attributes' => [
                    [
                        ...
                        FORMAT_mawang_COLLAPSED => new lang_string('showcollapsed', 'format_mawang'),
                    ],
                ],
                'cache' => true,
                'cachedefault' => FORMAT_mawang_EXPANDED,
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            $courseformatoptions = [
                'showsection0title' => [
                    'default' => get_config('format_mawang', 'showsection0titledefault') ?? 0,
                    'type' => PARAM_BOOL,
                ],
                'courseindexdisplay' => [
                    'default' => get_config('format_mawang', 'courseindexdisplay') ?? 0,
                    'type' => PARAM_INT,
                ],
                'cmbacklink' => [
        ...
        if ($foreditform && !isset($courseformatoptions['showsection0title']['label'])) {
            ...
            $courseformatoptionsedit = [
                'showsection0title' => [
                    'label' => new lang_string('showsection0title', 'format_mawang'),
                    'help' => 'showsection0title',
                    'help_component' => 'format_mawang',
                    'element_type' => 'advcheckbox',
                ],
                'courseindexdisplay' => [
                    'label' => new lang_string('courseindexdisplay', 'format_mawang'),
                    'element_type' => 'select',
     ...
     *
     * format_mawang processes additional attributes in the view course URL
    ...
    public function page_set_course_DISABLED(moodle_page $page) {
        ...
        if ($this->on_course_view_page()) {
            ...
            if ($switchcollapsed && confirm_sesskey() && has_capability('moodle/course:update', $context)
                ...
                if ($section->collapsed == FORMAT_mawang_EXPANDED) {
                    $newvalue = FORMAT_mawang_COLLAPSED;
                } else {
                    $newvalue = FORMAT_mawang_EXPANDED;
                }
                $this->update_section_format_options(['id' => $section->id, 'collapsed' => $newvalue]);
                if ($newvalue == FORMAT_mawang_COLLAPSED) {
                    if (!isset($options['sr'])) {
